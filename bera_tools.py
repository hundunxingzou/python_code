...
class BeraChainTools(object):
    def __init__(self, private_key, client_key='', solver_provider='', rpc_url='https://artio.rpc.berachain.com/'):
        ...
        if solver_provider not in ["yescaptcha", "2captcha"]:
            raise ValueError(
                "solver_provider must be 'yescaptcha' or '2captcha' or 'ez-captcha' ")
        ...
        self.bera_name_contract = self.w3.eth.contract(
            address=bera_name_address, abi=bera_name_abi)
    ...
    def get_2captcha_google_token(self) -> Union[bool, str]:
        ...
        response = requests.get(
            f'https://2captcha.com/in.php?', params=params).json()
    ...
    def get_ez_captcha_google_token(self) -> Union[bool, str]:
        ...
        response = self.session.post(
            url='https://api.ez-captcha.com/createTask', json=json_data).json()
        if response['errorId'] != 0:
            raise ValueError(response)
        task_id = response['taskId']
        time.sleep(5)
        for _ in range(30):
            data = {"clientKey": self.client_key, "taskId": task_id}
            response = requests.post(
                url='https://api.ez-captcha.com/getTaskResult', json=data).json()
            if response['status'] == 'ready':
                return response['solution']['gRecaptchaResponse']
            else:
                time.sleep(2)
    ...
    def approve_token(self, spender: Union[Address, ChecksumAddress], amount: int,
        ...
        if allowance_balance < amount:
            ...
            order_hash = self.w3.eth.send_raw_transaction(
    ...
    def bex_swap(self, amount_in: int, asset_in_address: Union[Address, ChecksumAddress],
        ...
        if asset_in_address == wbear_address:
            ...
            asset_in_token_contract = self.w3.eth.contract(
                address=asset_in_address, abi=erc_20_abi)
            balance = asset_in_token_contract.functions.balanceOf(
        ...
        for index, info in enumerate(swaps_list):
            swaps.append(dict(
                ...
                amountOut=0 if index +
                1 != len(swaps_list) else int(int(info['amountOut']) * 0.5),
                userData=b''))
    ...
    def bex_add_liquidity(self, amount_in: int, pool_address: Address, asset_in_address: Address) -> str:
        """
        bex 增加流动性
        :param amount_in: 输入数量
        ...
        asset_in_token_contract = self.w3.eth.contract(
            address=asset_in_address, abi=erc_20_abi)
        token_balance = asset_in_token_contract.functions.balanceOf(
            self.account.address).call()
    ...
    def honey_redeem(self, amount_honey_in: int) -> str:
        ...
        :return:
        """
        honey_balance = self.honey_contract.functions.balanceOf(
            self.account.address).call()
    ...
    def bend_deposit(self, amount_in: int, amount_in_token_address: Address) -> str:
        """
        bend deposit
        :param amount_in: 数量
        ...
        token_balance = amount_in_token_contract.functions.balanceOf(
            self.account.address).call()
        assert token_balance != 0
        assert token_balance >= amount_in
    ...
    def bend_borrow(self, amount_out: int, asset_token_address: Address) -> str:
        """
        bend borrow
        :param amount_out: 数量
    ...
    def bend_repay(self, repay_amount: int, asset_token_address: Address) -> str:
        """
        bend 还款
        ...
        """
        allowance_balance = self.honey_contract.functions.allowance(
            self.account.address, bend_address).call()
    ...
    def honey_jar_mint(self):
        allowance_balance = self.honey_contract.functions.allowance(
            self.account.address, ooga_booga_address).call()
        if allowance_balance / 1e18 < 4.2:
            raise ValueError(
                f'需要授权\nplease run : \nbera.approve_token(ooga_booga_address, int("0x" + "f" * 64, 16), "{honey_address}")')
        has_mint = self.ooga_booga_contract.functions.hasMinted(
            self.account.address).call()
        if has_mint:
            return True
        signed_txn = self.w3.eth.account.sign_transaction(
    ...
    def create_bera_name(self):
        random_str = ''.join(random.choice(emoji_list)
